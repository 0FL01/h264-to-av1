...
# H264 → AV1/H265 Video Converter

Набор скриптов для автоматического перекодирования H264 (MP4/MKV) с аппаратным ускорением:

| Скрипт | Платформа | Кодек | Энкодер |
|--------|-----------|-------|---------|
| `convert_linux.py` | Linux | AV1 | VAAPI (av1_vaapi) |
| `convert_windows.py` | Windows | H265/HEVC | NVENC / QSV / AMF / libx265 |

Скрипты автоматически подбирают битрейт под исходник, сохраняют баланс качества и размера, работают интерактивно и безопасно (атомарные записи, очистка временных файлов).
...
## Windows: H264 → H265/HEVC

### Особенности Windows-версии
- Поддержка нескольких аппаратных энкодеров с автоматическим определением:
  - **NVENC** (NVIDIA) — `hevc_nvenc` — быстрый, хорошее качество
  - **AMF** (AMD) — `hevc_amf` — быстрый, хорошее качество
  - **QSV** (Intel QuickSync) — `hevc_qsv` — быстрый, среднее качество
  - **D3D12VA** — `hevc_d3d12va` — универсальный, Windows 10+ (DirectX 12)
  - **Software** — `libx265` — медленный, лучшее качество (CPU fallback)
- Суффикс выходного файла: `-h265.mkv`
- Включение ANSI-цветов для Windows 10+

### Установка ffmpeg на Windows
1. Скачайте ffmpeg с одного из источников:
...
```
5. Проверьте работу конкретного энкодера (пример для AMD AMF):
```cmd
ffmpeg -f lavfi -i color=c=black:s=64x64:d=0.1:r=30 -c:v hevc_amf -f null -
```
Если команда завершилась без ошибок — энкодер работает.

### Запуск Windows-версии
```cmd
python convert_windows.py
```
При первом запуске скрипт автоматически определит доступные энкодеры и предложит выбрать один из них.
...
### H265 (Windows)
1. Аналогичный анализ через `ffprobe`.
2. Степенная кривая `target = 5.5 * bitrate^0.78` (менее агрессивная, т.к. H265 менее эффективен чем AV1):
...
### Windows (H265)
- **Не найден энкодер?** Убедитесь, что ffmpeg добавлен в PATH и поддерживает нужный энкодер (`ffmpeg -encoders | findstr hevc`).
- **NVENC не работает?** Обновите драйверы NVIDIA. Требуется видеокарта с поддержкой HEVC (GTX 900+, RTX).
- **QSV не работает?** Установите Intel Media Driver и проверьте, что CPU поддерживает QuickSync (Intel Core 6th gen+).
- **AMF не работает?** Обновите драйверы AMD. Требуется RX 400+ или APU Ryzen.
- **D3D12VA не работает?** Требуется Windows 10+ и GPU с поддержкой DirectX 12. Обновите драйверы GPU.
- **Энкодер показан как «недоступен»?** Скрипт тестирует каждый энкодер реальным кодированием. Если драйвер/железо не поддерживает энкодер, он будет пропущен.
- **Медленная конвертация?** Выберите аппаратный энкодер вместо libx265.
- **Хотите изменить качество NVENC?** Измените параметр `-preset` (p1-p7) и `-aq-strength` (1-15) в функции `build_encoder_params`.
