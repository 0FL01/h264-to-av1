# ffmpeg 8.0.1 (linuxserver/ffmpeg:8.0.1) — поддержка кодеков/железа

Использован контейнер с ENTRYPOINT=ffmpeg. Запуск на хосте с AMD Radeon 780M (RADV) и `/dev/dri` проброшенным в контейнер.

## Выполненные команды
- `docker run --rm --device=/dev/dri:/dev/dri -e RADV_PERFTEST=video_decode linuxserver/ffmpeg:8.0.1 -hide_banner -hwaccels`
- `docker run --rm --device=/dev/dri:/dev/dri -e RADV_PERFTEST=video_decode linuxserver/ffmpeg:8.0.1 -hide_banner -encoders`
- `docker run --rm --device=/dev/dri:/dev/dri -e RADV_PERFTEST=video_decode linuxserver/ffmpeg:8.0.1 -hide_banner -decoders`
- `docker run --rm --device=/dev/dri:/dev/dri -e RADV_PERFTEST=video_decode linuxserver/ffmpeg:8.0.1 -hide_banner -loglevel verbose -init_hw_device vulkan=vk:0 -f lavfi -i color=size=128x128:rate=1 -frames:v 1 -f null -`

## Краткие выводы (актуально для AMD RADV)
- hwaccels (полный список в билде, на AMD применимо): `vulkan`, `vaapi`, `vdpau`, `drm`, `opencl`. `cuda`/`qsv` присутствуют в сборке, но не релевантны для данного GPU.
- AV1 энкодеры: аппаратные `av1_vulkan`, `av1_vaapi`; софт `libaom-av1`, `librav1e`, `libsvtav1`. (Исключены Intel QSV/NVIDIA NVENC).
- Другие HW энкодеры: Vulkan `h264_vulkan`, `hevc_vulkan`, `ffv1_vulkan`; VAAPI `h264_vaapi`, `hevc_vaapi`, `vp8_vaapi`, `vp9_vaapi`, `mjpeg_vaapi`.
- AV1 декодеры: `av1` (native), `libdav1d`, `libaom-av1`. HW-деки, актуальные не для NVIDIA/Intel, опущены; из общих можно использовать v4l2m2m: `h264_v4l2m2m`, `hevc_v4l2m2m`.
- Vulkan инициализация прошла успешно: выбран GPU `AMD Radeon 780M Graphics (RADV PHOENIX)`; доступные очереди включают `decode` и `encode`. Распознаны расширения `VK_KHR_video_queue`, `VK_KHR_video_encode_queue`, `VK_KHR_video_decode_queue`, а также encode/decode для AV1/H264/H265/VP9.

## Фрагменты вывода (очищено)
```text
Hardware acceleration methods (build):
vdpau
vaapi
drm
opencl
vulkan      # основное для AMD RADV
```

```text
Encoders (AV1/Vulkan/VAAPI/прочие HW, отфильтровано под AMD):
libaom-av1
librav1e
libsvtav1
av1_vaapi
...
```text
Decoders (AV1 + HW, отфильтровано под AMD):
dav1d (libdav1d)
av1 (native)
libaom-av1
h264_v4l2m2m
...
```text
Vulkan init (RADV):
GPU 0: AMD Radeon 780M Graphics (RADV PHOENIX)
Using device extensions: VK_KHR_video_queue, VK_KHR_video_encode_queue, VK_KHR_video_decode_queue,
VK_KHR_video_encode_av1, VK_KHR_video_decode_av1, VK_KHR_video_encode_h264/265, VK_KHR_video_decode_h264/265,
VK_KHR_video_decode_vp9, VK_EXT_descriptor_buffer, VK_EXT_image_drm_format_modifier, ...
Queue families: graphics, compute, decode (1), encode (1)
```
...
```sh
➜  ~ docker run --rm --device=/dev/dri:/dev/dri -e RADV_PERFTEST=video_decode linuxserver/ffmpeg:8.0.1 -hide_banner -filters
**** creating video group videobmm8 with id 105 ****
**** adding /dev/dri/renderD128 to video group videobmm8 with id 105 ****
